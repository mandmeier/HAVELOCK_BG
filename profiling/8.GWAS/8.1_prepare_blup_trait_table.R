...
blup <- blup[match(order, blup$genotype),]
blup <- blup[, -1]

## remember which trait is which ASV
traits <- data.frame("ASV" = colnames(blup), "trait" = paste0("T", c(1:ncol(blup))))
## add to group data
...
### save trait data
write.table(blup, file = "cache/GWAS/traits_150_stdN.txt", sep = "\t", col.names = FALSE, row.names = FALSE)
...
### save trait data
write.table(blup, file = "cache/GWAS/traits_150_lowN.txt", sep = "\t", col.names = FALSE, row.names = FALSE)
