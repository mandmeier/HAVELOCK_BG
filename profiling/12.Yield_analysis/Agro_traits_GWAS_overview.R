## yield analysis using 15 agronomic traits
library("tidyverse")
...
files <- list.files(path="largedata/Agro_Traits_GWAS", pattern="*.txt", full.names=TRUE, recursive=FALSE)
dummy <- read_delim(file = "largedata/Agro_Traits_GWAS/LN-K.assoc.txt", delim = "\t")

Agro_GWAS <- filter(dummy, ps == "x")
Agro_GWAS$trait <- ""
Agro_GWAS$nitrogen <- ""

for(file in files){
  #file <- "largedata/Agro_Traits_GWAS/LN-K.assoc.txt"
  ...
  gwasdat$trait <- trait
  gwasdat$nitrogen <- N
  Agro_GWAS <- rbind(Agro_GWAS, gwasdat)

}

#save(Agro_GWAS, file="cache/Agro_traits_GWAS/Agro_GWAS.rda")

load("cache/Agro_traits_GWAS/Agro_GWAS.rda")

unique(Agro_GWAS$trait)


length(unique(Agro_GWAS$rs))
...
plot_dat <- gwas_dat %>%
  ...
  add_count(count = sum(log10p >= 5), name="total_sign_snps") %>%
  mutate(total_mean_p = mean(log10p)) %>%
  group_by(chr, bin, nitrogen, trait) %>%
  add_count(count = sum(log10p >= 5), name="sign_snps") %>%
...
agro_gwas_overview <- plot_dat


#save(agro_gwas_overview, file="cache/Agro_traits_GWAS/agro_gwas_overview.rda")
...
#load("cache/Agro_traits_GWAS/microbe_gwas_overview.rda")
