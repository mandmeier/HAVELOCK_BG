...
#### label top heritable taxa in +N, -N ####



## mark if top in both +N and -N

stdN_top <- filter(combined, H2 > 0.4 & nitrogen == "+N")$tax_group
lowN_top <- filter(combined, H2 > 0.4 & nitrogen == "-N")$tax_group
both_top <- Reduce(intersect, list(stdN_top, lowN_top))


combined$top_heritable <- "low heritability"

combined <- combined %>%
  mutate(top_heritable = replace(top_heritable, tax_group %in% stdN_top, "heritable under +N")) %>%
  mutate(top_heritable = replace(top_heritable, tax_group %in% lowN_top, "heritable under -N")) %>%
  mutate(top_heritable = replace(top_heritable, tax_group %in% both_top, "heritable under both"))

to_label <- filter(combined, top_heritable != "low heritability")$tax_group




h2_vs_blup <- ggplot(combined, aes(x = mean_blup, y = H2, labels=top_heritable, color=top_heritable)) +
  geom_point() +
  geom_label_repel(aes(label=ifelse(tax_group %in% to_label, tax_group, '')), size=2.5, min.segment.length=0.1) +
  scale_color_manual("tax group", values = c("#ff0000", "#0000ff", "#800080", "#999999")) +
  xlab("mean BLUP [log(relative abundance)] ") +
...
h2_vs_blup <- ggplot(combined, aes(x = exp(mean_blup), y = H2, labels=top_heritable, color=top_heritable)) +
  geom_point() +
  geom_label_repel(aes(label=ifelse(tax_group %in% to_label, tax_group, '')), size=2.5, min.segment.length=0.1) +
  scale_color_manual("tax group", values = c("#ff0000", "#0000ff", "#800080", "#999999")) +
